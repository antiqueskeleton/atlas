...
class DeepSeekProvider(AIProvider):
    ...
    def ask(self, prompt: str, context: str | None = None) -> AIReasoning:
        if not self.api_key:
            return AIReasoning(
                executive_summary="DeepSeek provider is selected but no API key is configured.",
                confidence="Low",
                risks=["Atlas cannot make a live DeepSeek request without an API key."],
                follow_up_questions=["Add a DeepSeek API key in Settings."],
        ...
        try:
            ...
            return AIReasoning(
                executive_summary=f"DeepSeek request failed ({self.model}): {error}",
                confidence="Low",
                risks=["The API key may be invalid or the request timed out."],
                follow_up_questions=[
                    "Check the DeepSeek API key in Settings.",
